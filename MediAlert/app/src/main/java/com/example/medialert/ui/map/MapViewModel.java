...
public class MapViewModel extends ViewModel {
    ...
    private void fetchAllTypes(Location userLocation) {
        new Thread(() -> {
            ...
            for (String type : new String[]{"hospital", "pharmacy", "clinic"}) {
    ...
    private void fetchType(Location userLocation, String type) {
        new Thread(() -> {
            List<PlaceItem> places = fetchPlacesForType(userLocation, type);
    ...
    private List<PlaceItem> fetchPlacesForType(Location userLocation, String type) {
        ...
        try {
            ...
            for (int i = 0; i < results.length(); i++) {
                ...
                placeList.add(new PlaceItem(name, address, lat, lng, distanceInKm));
            }
